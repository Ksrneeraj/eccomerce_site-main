...
const Contact = require("../models/productlist_women");
...
const createList = asyncHandler(async (req, res) => {
  ...
  const { image,image2,image3,title,description,price,discount,color,size,category,gender,query } = req.body;
  ...
  try{
    ...
    const user=new Contact({ image,image2,image3,title,description,price,discount,color,size,category,gender,query});
    await user.save();
...
const filter=asyncHandler(async(req,res)=>{
  ...
  "Green"
    ...
    let sortBy={};
      ...
      const documents=await Contact.find({query:{$regex:search,$options:"i"}})
        .where("color")
        .in([...color])
        .sort(sortBy)
